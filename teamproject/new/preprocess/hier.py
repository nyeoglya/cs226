...
class GeoHierDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        geo1 = torch.tensor(int(row["geo_level_1_id"]), dtype=torch.long)
        geo2 = torch.tensor(int(row["geo_level_2_id"]), dtype=torch.long)
        geo3 = torch.tensor(int(row["geo_level_3_id"]), dtype=torch.long)
        ...
        # 레이블 열이 정의된 경우에만 레이블을 추가
        if self.label_col is not None:
            y = int(row[self.label_col])
            data["label"] = torch.tensor(y, dtype=torch.long)
...
def hier_embed(df, model, device, batch_size=1024):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Extracting"): 
            x1 = batch["geo1"].to(device)
            x2 = batch["geo2"].to(device)
            x3 = batch["geo3"].to(device)
